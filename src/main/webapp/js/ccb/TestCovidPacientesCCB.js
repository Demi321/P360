...
/* global WebSocketGeneral, Promise, DEPENDENCIA, GenerarCredenciales, google */

modalImagenes();
habilitarMaximizarVideo();
var dataAgenda = {};
var $input;
fechas_testCovid_pacientes().then(function (agenda) {
        ...
        try {
            if (mensaje.inicializacionSG) {
                ...
                var ms = {
                    ...
                    "backup_pacientes_testcovid": true
                };
                EnviarMensajePorSocket(ms);
            }
            if (mensaje.pacientes_testcovid) {
        ...
        close: function () {
            ...
            if (fecha_calendario === getFecha()) {
                ...
                if (dataAgenda.hasOwnProperty(fecha_calendario)) {
                    ...
                    var ms = {
                        "fecha_calendario": fecha_calendario,
                        "backup_pacientes_testcovid_fecha": true
                    };
                    EnviarMensajePorSocket(ms);
                }
...
//        }
        ...
        $("#rtemperatura").text(json.check_temperatura === "0" ? "No" : "Si");
        if (json.check_temperatura === "1") {
            $("#div_pque_temp").removeClass("d-none");
            $("#div_rque_temp").removeClass("d-none");
            $("#rque_temp").text(json.resultado_temperatura);
        } else {
            $("#div_pque_temp").addClass("d-none");
            $("#div_rque_temp").addClass("d-none");
        }
        $("#roxigeno").text(json.check_oxigeno === "0" ? "No" : "Si");
        if (json.check_oxigeno === "1") {
            $("#div_poxigeno_resultado").removeClass("d-none");
            $("#div_roxigeno_resultado").removeClass("d-none");
            $("#roxigeno_resultado").text(json.resultado_oxigeno);
        } else {
            $("#div_poxigeno_resultado").addClass("d-none");
            $("#div_roxigeno_resultado").addClass("d-none");
            $("#roxigeno_resultado").text("");
        }
        $("#rdificultad_respirar").text(json.dificultad_respirar === "0" ? "No" : "Si");
        $("#rchifla_pecho").text(json.chifla_respirar === "0" ? "No" : "Si");
        $("#rrespiracion_frecuente").text(json.frecuencia_respirar === "0" ? "No" : "Si");
        $("#rdesorientado").text(json.desorientacion === "0" ? "No" : "Si");
        $("#rsangrado").text(json.sangrado === "0" ? "No" : "Si");
...
$("#guardarDiagnostico").on("click", function () {
    ...
    if ($(".textareaDiag").val() !== "") {
         ...
         */
        diagnostico_ccb(json).then(function (response) {
...
function fechas_testCovid_pacientes() {
    return Promise.resolve($.ajax({
        type: 'GET',
        url: '/' + DEPENDENCIA + '/API/test/fechas_testCovid_pacientes',
...
function info_fechas_testCovid_pacientes(fecha) {
    return Promise.resolve($.ajax({
        type: 'GET',
        url: '/' + DEPENDENCIA + '/API/test/fechas_testCovid_pacientes/' + fecha,
        contentType: "application/json;charset=UTF-8",
        success: function (response) {
            ////console.info(response);|
...
function diagnostico_ccb(json) {
    console.log(json);
    return Promise.resolve($.ajax({
        type: 'POST',
        url: '/' + DEPENDENCIA + '/API/diagnostico_ccb',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function actualizacion_pacientesTestCOVID(json) {
    console.log(json);
    return Promise.resolve($.ajax({
        type: 'POST',
        url: '/' + DEPENDENCIA + '/API/actualizacion_pacientesTestCOVID',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify(json),
        success: function (response) {
...
//            }
        ...
        streamCreated: function (event) {
            ...
            actualizacion_pacientesTestCOVID(Credenciales).then(function (response) {
                console.log(response);
