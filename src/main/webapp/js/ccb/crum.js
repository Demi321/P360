...
function agregar_bandeja(json) {
    ...
    div1.addEventListener("click", function () {
        ...
        aceptar.addEventListener("click", function () {
            ...
            ConsultarDirectorioCRUM().then(function (directorio) {
                Swal.fire({
                    title: 'Asignación',
                    ...
                    if (result.value) {
                        ...
                        json.personal_ambulancia = personal_ambulancia;
                        if (tel_a_agregar.length > 0) {
                            if (tel_a_agregar[0].hasOwnProperty("Nombre")) {
                                json.responsable_ambulancia_asignado = tel_a_agregar[0].Nombre;
                            } else {
                                json.responsable_ambulancia_asignado = "";
                            }
                        } else {
...
//        console.log(card[0].className);
        /*
         ...
         - NoingresoenUTC-19
         */
        if (card[0].className.includes("IngresadoenUTC-19") || card[0].className.includes("AceptadaporSUCRE")) {
            $("#botones").empty();
        } else {
            if (card[0].className.includes("AceptadaporCRUM")) {
                $("#botones").empty();
...
function cambio_estado_crum(json) {
    ...
    return Promise.resolve($.ajax({
        type: 'POST',
        url: '/' + DEPENDENCIA + '/API/cambio_estado_crum',
...
function ConsultarDirectorioCRUM() {
    return Promise.resolve($.ajax({
        type: 'GET',
        url: '/' + DEPENDENCIA + '/API/directorioCRUM',
        contentType: "application/json",
        success: function (response) {
