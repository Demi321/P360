/* global OT, API_KEY, TOKEN, SESSION_ID, SAMPLE_SERVER_BASE_URL, rectangle, Promise, markers, DEPENDENCIA, PREFIJO_ELEMENTO, CP, map, google, websocket, hostdir, ALIAS, MSJ, DEPENDENCIA_ALIAS, DEPENDENCIA_ICON, infowindow, recordRTC, INCIDENTES, direccion, USR, BackupImage, data, RequestPOST, Swal */
...
data.RegistroLlamada.time.h_atencion_inicio = getHora();
        ...
        if (!error) {
            ...
            session.publish(publisher, function publishCallback(publishErr) {
                if (publishErr) {
                    ...
                    data.RegistroLlamada.time.h_conexion_operador = getHora();
...
//                    CardParticipante(gps_data);
    ...
    session.connect(data.Credenciales.token, function callback(error) {
        ...
        if (!error) {
            // Text chat
            //añadimos la card del participante
            let info_data = JSON.parse($("#data").val());
            let ids_participantes = Object.keys(info_data.RegistroLlamada.participantes);
            console.log(ids_participantes);
            $.each(ids_participantes,(i)=>{
                let json = {
                    idUsuario_Movil:ids_participantes[i]
                };
                CardParticipante(json);
            });
...
function RegistrarNuevoParticipante(idUsuarios_Movil, firebase) {
    ...
    RegistroNotificaciones(idUsers, data.RegistroLlamada.idLlamada).then(function (RespuestaNotificados) {
...
function RegistrarURL(ruta_video) {
    data.RegistroLlamada.ruta_video = ruta_video;
    ...
    var settings = {
        ...
        "data": JSON.stringify({
            "idLlamada": data.RegistroLlamada.idLlamada,
            "idSys": data.RegistroLlamada.idOperador,
            "ruta_video": ruta_video,
            "hora_conexion": data.RegistroLlamada.time.h_conexion_operador,
            "hora_inicio": data.RegistroLlamada.time.h_atencion_inicio,
        }),
        success: function (response) {
...
$("#btn-reporte").click(function () {
   let notas = $("#notas").val();
   let acuerdos_conclusiones = $("#acuerdos_conclusiones").val();
   let procede = true;
   if (notas === "") {
        procede = false;
        $("#motivo").css("border", "solid 1px red");
    }
    if (acuerdos_conclusiones === "") {
        procede = false;
    ...
    if (procede) {
        var json = {
            "notas": notas,
            "acuerdos_conclusiones": acuerdos_conclusiones,
            "idLlamada": data.RegistroLlamada.idLlamada,
            "hora": getHora(),
            "fecha": getFecha(),
        ...
        RequestPOST("/API/empresas360/guardar_minuta_llamada_saliente",json).then((response)=>{
            console.log(response);
            if (response.success) {
                id_minuta = response.id_minuta;
                const Toast = Swal.mixin({
                    toast: true,
                    position: 'center',
                    showConfirmButton: false,
