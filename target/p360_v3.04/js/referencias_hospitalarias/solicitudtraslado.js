...
RequestGET("/API/referencia_hospitalaria/directorio_referencias/"+JSON.parse(getCookie("username_v3.1_" + DEPENDENCIA)).tipo_servicio).then((response)=>{
    ...
    for (let i = 0; i < response.length; i++) {
        ...
        $("#listado_instituciones_referencia").append(option);
    }
});
...
function solicitud_traslado(json) {
    ...
    json.to_tipo_usuario_institucion = $("#listado_instituciones_referencia").val().split("-")[0];
    json.to_tipo_servicio_institucion = $("#listado_instituciones_referencia").val().split("-")[1];
    
    return Promise.resolve($.ajax({
        type: 'POST',
        url: '/' + DEPENDENCIA + '/API/referencias_hospitalarias/solicitud_traslado',
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        "data": JSON.stringify(json),
        success: function (response) {
